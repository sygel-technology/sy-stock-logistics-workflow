...
from odoo.addons.queue_job.job import job, DONE
...
class StockPicking(models.Model):
    ...
    def write(self, vals):
        ...
        for sel in self:
            ...
            if sel.carrier_id or vals.get('carrier_id'):
                sel.set_queues_to_done('Set to done when selecting carrier', 'carrier_id')
            elif not sel.purchase_shipping_date and not vals.get('purchase_shipping_date'):
                sel.set_queues_to_done('Set to done when removing shipping date', 'carrier_id')
            if sel.purchase_id and not sel.carrier_id and not vals.get('carrier_id') and sel.state not in ['done', 'cancel', 'draft']:
                sel.set_queues_to_done('Set to done when selecting shipping date', 'carrier_id')
            ...
            if sel.purchase_shipping_date or vals.get('purchase_shipping_date'):
                sel.set_queues_to_done('Set to done when selecting purchase shipping date', 'shipping_date')
            if sel.purchase_id and not sel.purchase_shipping_date and not vals.get('purchase_shipping_date') and sel.state not in ['done', 'cancel', 'draft']:
                sel.set_queues_to_done('Set to done when selecting shipping date', 'shipping_date')
        ...
        for sel in self.filtered(
            ...
            if sel.picking_type_id.code == 'incoming' and sel.purchase_id:
                ...
                for mail in order_type.picking_mail_notify_ids.filtered(
                    ...
                    triggering_date = sel.calculate_trigger_time(mail.reference_field, mail.notify_when, mail.notify_time)
                    if triggering_date and triggering_date > date_now:
                        sel.with_delay(eta=triggering_date).delay_picking_mail_notify_carrier(mail)
                for log in order_type.picking_log_note_notify_ids.filtered(
                    lambda a: a.empty_field == 'carrier_id'
                ):
                    triggering_date = sel.calculate_trigger_time(log.reference_field, log.notify_when, log.notify_time)
                    if triggering_date and triggering_date > date_now:
                        sel.with_delay(eta=triggering_date).delay_picking_log_notify_carrier(log)
                for activity in order_type.picking_activity_notify_ids.filtered(
                    lambda a: a.empty_field == 'carrier_id'
                ):
                    triggering_date = sel.calculate_trigger_time(activity.reference_field, activity.notify_when, activity.notify_time)
                    if triggering_date and triggering_date > date_now:
                        # Le paso el triggering date para utilizarlo como referencia
                        # para hacer la acción planificada
                        sel.with_delay(eta=triggering_date).delay_picking_activity_notify_carrier(activity, triggering_date)
        ...
        for sel in self.filtered(
            ...
            if sel.picking_type_id.code == 'incoming' and sel.purchase_id:
                ...
                for mail in order_type.picking_mail_notify_ids.filtered(
                    ...
                    triggering_date = sel.calculate_trigger_time(mail.reference_field, mail.notify_when, mail.notify_time)
                    if triggering_date and triggering_date > date_now:
                        sel.with_delay(eta=triggering_date).delay_picking_mail_notify_shipping_date(mail)
                for log in order_type.picking_log_note_notify_ids.filtered(
                    lambda a: a.empty_field == 'purchase_shipping_date'
                ):
                    triggering_date = sel.calculate_trigger_time(log.reference_field, log.notify_when, log.notify_time)
                    if triggering_date and triggering_date > date_now:
                        sel.with_delay(eta=triggering_date).delay_picking_log_notify_shipping_date(log)
                for activity in order_type.picking_activity_notify_ids.filtered(
                    lambda a: a.empty_field == 'purchase_shipping_date'
                ):
                    triggering_date = sel.calculate_trigger_time(activity.reference_field, activity.notify_when, activity.notify_time)
                    if triggering_date and triggering_date > date_now:
                        # Le paso el triggering date para utilizarlo como referencia
                        # para hacer la acción planificada
                        sel.with_delay(eta=triggering_date).delay_picking_activity_notify_shipping_date(activity, triggering_date)
    ...
    def delay_picking_activity_notify_carrier(self, activity, triggering_date):
        if not self.carrier_id and self.state == 'assigned':
            deadline = fields.Datetime.to_string(triggering_date + timedelta(days=activity.time_to_deadline))
    ...
    def set_queues_to_done(self, message, field=False):
        for sel in self:
            if field:
                channel = 'root.picking_notify_carrier' if field == 'carrier_id' else 'root.picking_notify_shipping_date'
                for notification in sel.picking_queue_ids.filtered(
                    lambda a: a.state not in ['done'] and a.channel == channel
                ):
                    notification._change_job_state(
                        DONE,
                        message
                    )
            else:
    ...
    def calculate_trigger_time(self, reference, when, time):
        triggering_date = self.scheduled_date if reference == 'scheduled_date' else self.purchase_shipping_date
