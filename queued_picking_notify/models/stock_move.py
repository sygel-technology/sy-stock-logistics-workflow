...
class StockMove(models.Model):
    ...
    def _compute_product_supplier_code(self):
        for move in self.filtered(
            lambda m: m.picking_id and m.picking_id.partner_id and m.product_tmpl_id.seller_ids
        ):
            suppliers = \
                move.product_tmpl_id.seller_ids.filtered(
                    lambda m: m.name.id == move.picking_id.partner_id.id or m.name.id == move.picking_id.partner_id.parent_id.id
                )
            if suppliers:
